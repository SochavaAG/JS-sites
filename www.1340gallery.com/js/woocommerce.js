(function($){
	 ...
	 // Ajax QuickView
		$(document).ready(function(){
			$('a.quickview').on('click', function (e) {
			    ...
			    var url = woocommerce_params.ajax_url + '?action=uruana_quickview&productslug=' + productslug;
			     $.get(url,function(data,status){
			     		$('#uruana-quickview-modal .modal-body').html(data);
			     });
			 });
			$('#uruana-quickview-modal').on('hidden.bs.modal',function(){
	...
	var engoproductcatid = null; 
    var product = null;

    jQuery('body').bind('adding_to_cart', function( button, data , data2 ){
       engoproductcatid =  data.data('product_id');
       product = data;

    });
    jQuery('body').bind('added_to_cart', function( fragments, cart_hash ){
        if( engoproductcatid ){
            var imgtodrag = $('[data-product-id="'+engoproductcatid+'"] .image img').eq(0);
            var cart =  $('#cart');
            if (imgtodrag) {
